...
import {
  requestPollinationsSprite,
  getSavedSprite,
  subscribeSpriteReady,
  isSpriteGenerating,
  ProcessedSpriteRecord,
} from './pollinations-sprite-pipeline';
import {
  computeCanonicalItemId,
  ItemResolutionInput,
} from './canonical-item';
...
export interface CachedSpriteData {
  ...
  createdAt: number;
  source: 'ai' | 'semantic' | 'pollinations';
  tags?: string[];
}
...
const SPRITE_CACHE_VERSION = 'v66_variety_10_15_steps';
...
export function getCachedSprite(name: string): CachedSpriteData | null {
  ...
  // 1. Check permanently saved Pollinations 64x64 sprite
  const saved = getSavedSprite(cleanName);
  if (saved) {
    return {
      name: saved.displayName || cleanName,
      canonicalId: saved.canonicalId,
  ...
  // 2. Check in-memory matrix cache
...
function getImageClient(): GoogleGenAI | null {
  ...
  try {
    ...
    console.warn('Failed to initialize GoogleGenAI client for sprite images:', err);
    return null;
  }
}
...
- if this is a process, depict the actual tool action/process rather than a symbol`;
  ...
  if (!interactionsApi || typeof interactionsApi.create !== 'function') {
    return null;
  }
...
export interface BackgroundSpriteOptions extends SpriteGenerationOptions {
  description?: string;
...
export function enqueueBackgroundSpriteGeneration(
  ...
  options: BackgroundSpriteOptions = {}
) {
  if (!name || !name.trim()) return;
  const cleanName = name.trim();

  // Non-blocking trigger of Pollinations 64x64 pixel art sprite pipeline
  requestPollinationsSprite({
    name: cleanName,
    category,
    emoji,
    ...
    ingredientHistory: options.ingredientHistory,
    processHistory: options.processHistory,
  }).catch(() => {
    // Pipeline handles retries and logging internally
  });
