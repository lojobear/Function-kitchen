...
export class GenAIGeminiClient extends EventEmitter<GeminiClientEventTypes> {
  ...
  constructor(options: GeminiClientOptions) {
    super();
    const key = options.apiKey || (typeof process !== 'undefined' && process.env?.API_KEY) || "placeholder_key";
    try {
      this.genAI = new GoogleGenAI({ apiKey: key });
    } catch (e) {
      console.warn("Gemini client initialized with fallback placeholder:", e);
      this.genAI = new GoogleGenAI({ apiKey: "placeholder_key" });
    }
